    ...
    <div className="py-16 md:py-24 px-4">
      ...
      <div className="mx-auto text-center" style={{ maxWidth: 640 }}>
        <div style={{ transition: "opacity 300ms ease", opacity: fade ? 1 : 0 }}>
          <p className="font-serif italic text-xl sm:text-2xl leading-relaxed mb-6" style={{ color: "hsl(var(--text-primary))", lineHeight: 1.6 }}>
            "{t.quote}"
          </p>
          <div className="flex items-center justify-center gap-3">
            <div className="rounded-full flex items-center justify-center text-sm font-bold" style={{ width: 40, height: 40, background: "hsl(var(--primary))", color: "hsl(var(--primary-foreground))", flexShrink: 0 }}>
              {t.name.charAt(0)}
            </div>
        ...
        <div className="flex justify-center gap-2 mt-8">
          {testimonials.map((_, i) => (
            <button key={i} onClick={() => { setFade(false); setTimeout(() => { setActiveIdx(i); setFade(true); }, 200); }} className="rounded-full transition-all duration-200" aria-label={`Testimonial ${i + 1}`} style={{ width: activeIdx === i ? 20 : 8, height: 8, background: activeIdx === i ? "hsl(var(--primary))" : "hsl(var(--border))" }} />
          ))}
        </div>
      </div>
    </div>
