...
export default function AccountSettingsPage() {
  ...
  const handleDeleteAccount = async () => {
    // Sign out and notify — actual deletion would require a backend function
    ...
    <>
      ...
      <div className="min-h-screen px-4 pt-28 pb-20 page-enter">
        <div style={{ maxWidth: 640 }} className="mx-auto">
          <h1 className="text-3xl font-bold mb-10" style={{ color: "hsl(var(--text-primary))" }}>
            Account Settings
          </h1>
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium" style={{ color: "hsl(var(--text-tertiary))" }}>Email</label>
                <p className="text-base mt-1" style={{ color: "hsl(var(--text-primary))" }}>{user.email}</p>
              </div>
              <div>
                <label className="text-sm font-medium" style={{ color: "hsl(var(--text-tertiary))" }}>Member since</label>
                <p className="text-base mt-1" style={{ color: "hsl(var(--text-primary))" }}>
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm" style={{ color: "hsl(var(--text-secondary))" }}>{tier.label}</p>
              </div>
              {subscription.plan === "free" ? (
                <button
                  onClick={() => navigate("/pricing")}
                  className="rounded-full px-5 py-2 text-sm font-semibold transition-all hover:scale-[1.02] active:scale-[0.98]"
                  style={{
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-full px-5 py-2 text-sm font-medium transition-all hover:scale-[1.02]"
                  style={{
            ...
            <div>
              ...
              <div
                ...
                style={{ height: 8, background: "hsla(0, 0%, 100%, 0.08)" }}
              >
                <div
                  ...
                  style={{
                    width: `${usagePercent}%`,
                    height: "100%",
                    background: usagePercent > 80 ? "hsl(var(--warning))" : "hsl(var(--primary))",
                  }}
                />
              </div>
            </div>
          ...
          <section
            ...
            style={{ background: "hsla(0, 84%, 60%, 0.05)", border: "1px solid hsla(0, 84%, 60%, 0.15)" }}
          >
            <div className="flex items-center gap-3 mb-4">
              <AlertTriangle className="w-5 h-5" style={{ color: "hsl(var(--destructive))" }} />
              <h2 className="text-lg font-semibold" style={{ color: "hsl(var(--destructive))" }}>Danger Zone</h2>
            </div>

            {!showDeleteConfirm ? (
              <button
                onClick={() => setShowDeleteConfirm(true)}
                className="rounded-xl px-5 py-3 text-sm font-medium transition-all hover:scale-[1.02]"
                style={{
