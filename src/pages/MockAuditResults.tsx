...
import { ArrowRight, ArrowLeft, ArrowUp, AlertTriangle, Users, Target, Shield, TrendingUp, Copy, Share2, Swords, CheckCircle } from "lucide-react";
...
const mockData = {
  ...
  stakeholders: [
    { name: "Board of Directors", stance: "Supportive", action: "Present updated financial model." },
    { name: "CTO / Engineering", stance: "Cautious", action: "Schedule joint architecture review." },
    { name: "Sales Team", stance: "Concerned", action: "Communicate territory protection." },
    { name: "Target's Employees", stance: "Uncertain", action: "Prepare Day 1 communication." },
  ],
  actions: [
    { action: "Commission independent due diligence on tech stack", priority: "Immediate" },
    { action: "Develop 18-month integration roadmap", priority: "This week" },
    { action: "Negotiate retention agreements with top 5 leaders", priority: "Before close" },
    { action: "Model three timeline scenarios", priority: "Immediate" },
  ],
  devilsAdvocate: "You're assuming the competitor's technology is worth the premium. But what if their engineering talent — the real asset — leaves within 6 months? You'd be left with a codebase you don't fully understand and a market position that could have been achieved organically at half the cost.",
  ...
  rapidAssignment: {
    R: { label: "Recommend", person: "VP Corp Dev" },
    A: { label: "Agree", person: "CFO, Legal" },
    P: { label: "Perform", person: "Integration PMO" },
    I: { label: "Input", person: "CTO, VP Sales, HR" },
    D: { label: "Decide", person: "CEO + Board" },
  },
};
...
function ScoreGauge({ score }: { score: number }) {
  ...
  useEffect(() => {
    setTimeout(() => setScaleIn(true), 100);
    const duration = 1500;
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ transform: scaleIn ? "scale(1)" : "scale(0.8)", opacity: scaleIn ? 1 : 0, transition: "transform 500ms ease-out, opacity 500ms ease-out" }}>
      ...
      <text x={size/2} y={size/2 - 5} textAnchor="middle" fill="hsl(var(--text-primary))" fontSize="56" fontWeight="800" style={{ fontFamily: "Inter" }}>{animatedScore}</text>
      <text x={size/2} y={size/2 + 22} textAnchor="middle" fill="hsl(var(--text-tertiary))" fontSize="14">/100</text>
    </svg>
  );
}
    ...
    <>
      ...
      <div className="min-h-screen px-4 sm:px-6 pt-24 pb-20 page-enter">
        <div className="mx-auto" style={{ maxWidth: 800 }}>
          ...
          {/* Score — 240px gauge, centered */}
          <div id="verdict-section" ref={scoreRef} className="rounded-xl p-6 text-center mb-6 scroll-mt-24" style={{ background: "hsla(173, 58%, 39%, 0.05)", border: "1px solid hsl(var(--border))" }}>
            <div className="flex justify-center"><ScoreGauge score={mockData.confidenceScore} /></div>
            <span className="inline-block mt-2 rounded-full px-4 py-1.5 text-sm font-bold" style={{
          ...
          {/* Executive Summary — 3 column strip */}
          ...
          {/* Copy Summary — prominent on mobile */}
          <div className="mb-6">
            <button onClick={handleCopySummary} className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-full px-6 py-3.5 text-sm font-semibold transition-all duration-200 hover:scale-[1.02] active:scale-[0.98]" style={{ background: "hsl(var(--primary))", color: "hsl(var(--primary-foreground))" }}>
          ...
          {/* Sticky Tab Nav */}
          <div className="sticky top-14 z-40 -mx-4 px-4 py-2 mb-6" style={{ background: "hsla(0, 0%, 100%, 0.95)", backdropFilter: "blur(8px)", borderBottom: "1px solid hsl(var(--border))" }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {mockData.stakeholders.map((s, i) => (
                <div key={i} className="rounded-lg p-3" style={{ background: "hsl(0, 0%, 100%)", border: "1px solid hsl(var(--border))" }}>
                  ...
                  <p className="text-xs" style={{ color: "hsl(var(--text-tertiary))" }}>{s.action}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div id="rapid" className="rounded-xl p-5 mb-4 scroll-mt-24" style={{ background: "hsl(var(--secondary))", border: "1px solid hsl(var(--border))" }}>
            ...
            <div className="flex flex-wrap gap-4 justify-between">
              {Object.entries(mockData.rapidAssignment).map(([letter, { label, person }]) => (
                <div key={letter} className="flex flex-col items-center text-center flex-1" style={{ minWidth: 60 }}>
