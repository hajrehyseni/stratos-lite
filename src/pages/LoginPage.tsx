...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    else { const params = new URLSearchParams(window.location.search); navigate(params.get("redirect") ? `/${params.get("redirect")}` : "/"); }
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const result = await lovable.auth.signInWithOAuth("google", { redirect_uri: redirect ? `${window.location.origin}/${redirect}` : window.location.origin });
      if (result.error) { const msg = String(result.error); toast.error(msg.includes("configuration") || msg.includes("provider") ? "Google login is not yet configured. Please use email/password to sign in." : msg); }
    } catch (e: any) { toast.error(e?.message || "Google login failed."); }
    finally { setGoogleLoading(false); }
  };

  const inputStyle = {
    background: "hsl(var(--secondary))", border: "1.5px solid hsl(var(--border))", height: 52, fontSize: 16, color: "hsl(var(--text-primary))",
  };
    ...
    <>
      ...
      <div className="min-h-screen flex items-center justify-center px-4 pt-16 page-enter" style={{ background: "hsl(var(--background))" }}>
        <div className="w-full" style={{ maxWidth: 420 }}>
          <h1 className="text-3xl font-bold mb-2" style={{ color: "hsl(var(--text-primary))" }}>Welcome back</h1>
          <p className="mb-8 text-lg" style={{ color: "hsl(var(--text-secondary))" }}>Your decisions are waiting.</p>

          <button onClick={handleGoogleLogin} disabled={googleLoading} className="w-full flex items-center justify-center gap-3 rounded-full transition-all duration-200 mb-6 hover:scale-[1.02] active:scale-[0.98]" style={{ height: 56, fontSize: 16, fontWeight: 600, background: "hsl(var(--primary))", color: "hsl(var(--primary-foreground))", opacity: googleLoading ? 0.7 : 1 }}>
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            <div>
              ...
              <input id="login-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" required className="w-full rounded-xl px-5 outline-none transition-all duration-200" style={inputStyle}
            ...
            <div>
              ...
              <input id="login-password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" required className="w-full rounded-xl px-5 outline-none transition-all duration-200" style={inputStyle}
                onFocus={(e) => { e.currentTarget.style.borderColor = "hsl(var(--primary))"; e.currentTarget.style.boxShadow = "0 0 0 3px hsla(221, 83%, 53%, 0.12)"; }}
                onBlur={(e) => { e.currentTarget.style.borderColor = "hsl(var(--border))"; e.currentTarget.style.boxShadow = "none"; }}
              />
              <Link to="/forgot-password" className="mt-2 inline-block text-base font-medium underline underline-offset-4 transition-opacity hover:opacity-80" style={{ color: "hsl(var(--primary))", minHeight: 44 }}>Forgot password?</Link>
            </div>
